...
int main(int argc,char** argv){
	...
	struct sembuf op1;
	op1.sem_num = 1; //Numéro de notre sémaphore: le premier et le seul
	op1.sem_flg = 0; //On ne s'en occupe pas
	...
	if((sem_id=semget(IPC_PRIVATE,2,IPC_CREAT|0666))<0){
...
// 	semctl(sem_id, 1, SETVAL, 1);
	...
	*cpt=0;
	...
	op1.sem_op = -1; //Pour un P() on décrémente
	semop(sem_id, &op1, 1);
	decrement();
	op1.sem_op = 1; //Pour un V() on incrémente
	semop(sem_id, &op1, 1);
