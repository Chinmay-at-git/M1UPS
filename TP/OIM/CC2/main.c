          ...
          char *argv[])
{
 ...
 loop = g_main_loop_new (NULL, FALSE);
  ...
  gst_bin_add_many (GST_BIN (pipeline),
                    ...
                    queueAudio, queueVideo, audioSink,videoSink, videoConv, 
NULL);

  /* On relie les elements entre eux */
  /* file-source -> ogg-demuxer ~> vorbis-decoder -> converter -> alsa-output */
  gst_element_link (source, demuxer);
  gst_element_link_many (queueVideo, videoDecoder, videoConv,videoSink, NULL);
  gst_element_link_many (queueAudio, audioDecoder , conv, audioSink, NULL);
